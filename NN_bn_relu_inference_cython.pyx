...
def bn_relu_inference_cython(x, running_mean, std, gamma, beta):
    ...
    if (x.dtype == np.int8):
        bn_relu_inference_cython_inner_int8(x, running_mean, std, y, gamma, beta)
    elif (x.dtype == np.float32):
        bn_relu_inference_cython_inner_float32(x, running_mean, std, y, gamma, beta)
    elif (x.dtype == np.float64):
        bn_relu_inference_cython_inner_float64(x, running_mean, std, y, gamma, beta)
    else:
        print("Type %s not supported for bn_relu_inference_cython!" % (str(x.dtype)))
        raise
