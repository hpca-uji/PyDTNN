...
class LayerNormalizationPycuda(LayerNormalization[TensorArray], LayerPycuda):
    ...
    def _model_init(self, prev_shape, x):
        ...
        # Shape same as x input, but last layer = 1. For mean computation across
        # the normalization axis.
        mean_shape = (
            int(np.prod(x.shape) / np.prod([x.shape[i] for i in self.axis])),
        )  # (*x.shape[:-2], 1, 1)
        ...
        self.blocks_backward_weights = int(
    ...
    def __init_kernels_gpu__(self):
        ...
        self.kernel_backward_weigths = self._get_kernel(
            func_name="layer_normalization_backward_weights"
        )
        return
