...
class ConcatenationBlockPycuda(ConcatenationBlock[TensorArray], AbstractBlockLayerPycuda):
    ...
    def _model_init(self, prev_shape: ArrayShape, x: TensorArray) -> None:
        ...
        self.concat = ElementwiseKernel(
            "{T} *dst, {T} *src, int N, int H, int W, int C, int first_c, int last_c".format(
        ...
        self.split = ElementwiseKernel(
            "{T} *src, {T} *dst, int N, int H, int W, int C, int first_c, int last_c".format(
                T=DTYPE2CTYPE[self.model.dtype]
            ),
            {
                TensorFormat.NHWC: (
    ...
    def forward(self, x: TensorArray) -> TensorArray:
        ...
        for i, p in enumerate(self.paths):
            ...
            self.model.tracer.emit_event(
                PYDTNN_OPS_EVENT, self.id * PYDTNN_OPS_EVENTS + PYDTNN_OPS_EVENT_enum.FORWARD_CONCAT
            )
            self.concat(
                self.y.ary,
                y_i.ary,
    ...
    def backward(self, dy: TensorArray) -> TensorArray:
        ...
        for i, p in enumerate(self.paths):
            self.model.tracer.emit_event(
                PYDTNN_OPS_EVENT, self.id * PYDTNN_OPS_EVENTS + PYDTNN_OPS_EVENT_enum.BACKWARD_SPLIT
            )
            self.split(
                dy.ary,
                self.dy[i].ary,
