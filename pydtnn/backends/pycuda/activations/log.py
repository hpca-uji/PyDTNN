    ...
    def _model_init(self, prev_shape: ArrayShape, x: TensorArray) -> None:
        ...
        self.log = ElementwiseKernel(
            ...
            "out[i] = {func_log}(1.0 / (1.0 + {func_exp}(-in[i])));".format(
                func_log={np.dtype(np.float32): "logf", np.dtype(np.float64): "log"}[
                    self.model.dtype
                ],
                func_exp={np.dtype(np.float32): "expf", np.dtype(np.float64): "exp"}[
                    self.model.dtype
                ],
            ),
            "log_Pycuda",
        ...
        self.dlog = ElementwiseKernel(
            "{T} *in, {T} *out".format(T=DTYPE2CTYPE[self.model.dtype]),
            "out[i] = 1.0 / (1.0 + {func}(in[i]));".format(
                func={np.dtype(np.float32): "expf", np.dtype(np.float64): "exp"}[self.model.dtype]
            ),
            "dlog_Pycuda",
        )
