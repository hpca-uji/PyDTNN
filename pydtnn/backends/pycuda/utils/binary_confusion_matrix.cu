...
__constant__ const short indexes[2][NUM_ROWS][NUM_COLS] = {
...
__global__ void binary_confusion_matrix(TYPE *y_targ, TYPE *y_pred, int *cm, int *local_cm, const int num_classes, const int n)
{
    ...
    for(idx = base_idx; idx < n; idx += workers)
    {
        for(label = 0; label < num_classes; label++)
        {
            ...
            *(local_cm + SHIFT_POINTER_LOCAL_CM(idx, label, index_0, index_1, num_classes, NUM_ROWS, NUM_COLS)) += 1;
        }
    }
    __syncthreads();
            ...
            for(label = 0; label < num_classes; label++)
                for(i = 0; i < NUM_ROWS; i++) 
                    for(j = 0; j < NUM_COLS; j++)
                        *(cm + SHIFT_POINTER_CM(label, i, j, 2, 2)) += *(local_cm + SHIFT_POINTER_LOCAL_CM(idx, label, i, j, num_classes, NUM_ROWS, NUM_COLS));
    }
}