...
class PrecisionPycuda(Precision[TensorArray], MetricPycuda):
    ...
    def _model_init(self) -> None:
        ...
        self.precision = TensorArray.new_zeros(shape=(1, ), dtype=self.model.dtype,
                                               tensor_format=self.model.tensor_format, cudnn_dtype=self.model.cudnn_dtype)
        self.local_precision = TensorArray.new_zeros(shape=(target_classes, ), dtype=self.model.dtype,
                                                     tensor_format=self.model.tensor_format, cudnn_dtype=self.model.cudnn_dtype)
