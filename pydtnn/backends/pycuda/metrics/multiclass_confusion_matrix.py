...
class MulticlassConfusionMatrixPycuda(MulticlassConfusionMatrix[TensorArray], MetricPycuda):
    ...
    def _model_init(self) -> None:
        ...
        self.conf_matrix = TensorArray.create_zeros_tensor(shape=(1, 1, target_classes, target_classes), dtype=np.dtype(np.int32),
                                                         tensor_format=self.model.tensor_format, cudnn_dtype=self.model.cudnn_dtype)
        self.local_cm = TensorArray.create_zeros_tensor(shape=(1, n, target_classes, target_classes), dtype=np.dtype(np.int32),
                                                      tensor_format=self.model.tensor_format, cudnn_dtype=self.model.cudnn_dtype)
    # ----
