...
class F1ScoreNumpy(F1Score[np.ndarray], MetricNumpy):
    ...
    def _post_init(self) -> None:
        ...
        with self.model.memory:
            self.true_positives = self.model.memory.ndarray(
                self.temp_var_shape, dtype=np.dtype(np.float32)
            )
            self.false_positives = self.model.memory.ndarray(
                self.temp_var_shape, dtype=np.dtype(np.float32)
            )
            self.false_negatives = self.model.memory.ndarray(
                self.temp_var_shape, dtype=np.dtype(np.float32)
            )
            self.are_zeros = self.model.memory.ndarray(
                self.temp_var_shape, dtype=np.dtype(np.bool_)
            )
