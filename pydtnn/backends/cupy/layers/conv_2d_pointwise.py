...
from pydtnn.backends.cupy.layers.abstract.conv_2d import Conv2DCUPY
...
class Conv2DPointwiseCUPY(Conv2DCUPY):
    ...
    def initialize(self, prev_shape: ArrayShape, x: np.ndarray | None = None) -> None:
        ...
        # self.dw (this one too, but it's initalized in Conv2DCUPY)
    ...
    def _backward_nhwc(self, dy: np.ndarray) -> np.ndarray:
        ...
        return np.asarray(dx.reshape(x_shape), dtype=self.model.dtype)
