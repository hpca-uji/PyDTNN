import cupy as np
from pydtnn.backends.cupy.utils.bn_training_cython import bn_training_bwd_cython  # , bn_training_fwd_cython
...
class BatchNormalizationCUPY(BatchNormalization[np.ndarray], LayerCUPY):
    ...
    def initialize(self, prev_shape: ArrayShape, x: np.ndarray | None = None):
        ...
        self._mean_inv: np.ndarray = np.zeros(shape=(self.ci,), dtype=self.model.dtype)
        self._var_inv: np.ndarray = np.zeros(shape=(self.ci,), dtype=self.model.dtype)
        self.std: np.ndarray = np.zeros(shape=(self.ci,), dtype=self.model.dtype)

        if self.spatial:
            self.dx: np.ndarray = np.zeros(shape=(self.model.batch_size * self.hi * self.wi, self.ci), dtype=self.model.dtype)
            self.y = np.zeros((self.model.batch_size * self.hi * self.wi, self.ci), dtype=self.model.dtype)
            self.dy_xn = np.zeros((self.model.batch_size * self.hi * self.wi, self.ci), dtype=self.model.dtype)
        else:
            # NOTE: in this case, self.hi and self.wi are 0 (self.shape should be somethin like: "(512, )"
            self.dx: np.ndarray = np.zeros(shape=(self.model.batch_size, self.ci), dtype=self.model.dtype)
            self.y = np.zeros((self.model.batch_size, self.ci), dtype=self.model.dtype)
            self.dy_xn = np.zeros((self.model.batch_size, self.ci), dtype=self.model.dtype)
    # --
