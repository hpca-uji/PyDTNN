...
class ReluCupy(ReluNumpy, ActivationCupy):
    ...
    def _model_init(self, prev_shape, x=None):
        ...
        self.fwd = self.relu_fwd()
        self.bwd = self.relu_bwd()
    ...
    def relu_fwd(self) -> np.RawKernel:
        func_name = "relu_fwd"
        code = \
            r"""
__global__ void {FUNC_NAME}({T}* x, {T}* max, {T}* mask, int N)
{{
    int i;
    {T} elem;
...
"""
    ...
    def relu_bwd(self) -> np.RawKernel:
        func_name = "relu_bwd"
        code = \
            r"""
