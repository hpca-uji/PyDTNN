...
from pydtnn.layers import layers
from pydtnn.model import EVALUATE_MODE
from ..tracers import PYDTNN_MDL_EVENT, \
    PYDTNN_MDL_EVENTS, \
    PYDTNN_MDL_FORWARD, \
    PYDTNN_MDL_BACKWARD, \
    PYDTNN_OPS_EVENT, \
...
class FCGPU(layers.FC):
    ...
    def initialize(self, prev_shape, need_dx, x):
        ...
        self.matmul, self.matvec = utils.matmul_gpu, utils.matvec_gpu
...
class MaxPool2DGPU(layers.MaxPool2D):
...
class AveragePool2DGPU(layers.AveragePool2D):
...
class DropoutGPU(layers.Dropout):
...
class FlattenGPU(layers.Flatten):
...
class BatchNormalizationGPU(layers.BatchNormalization):
...
class AdditionBlockGPU(layers.AdditionBlock):
...
class ConcatenationBlockGPU(layers.ConcatenationBlock):
