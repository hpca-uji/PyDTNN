...
class Model:
    ...
    def _train_batch(self, x_batch, y_batch, current_batch_size, current_epoch, current_step):
        ...
        if self.blocking_mpi:
            ...
            for i in range(len(self.layers) - 1, 0, -1):
                ...
                self.layers[i].update_weights(self.optimizer, current_step=current_step, current_epoch=current_epoch)
                self.tracer.emit_event(PYDTNN_MDL_EVENT, 0)
        else:
            # Non-blocking MPI
            # Back propagation. Gradient computation (GC) and weights update (WU)
    ...
    def train_dataset(self, bar_width=110):
        ...
        for epoch in range(self.num_epochs):
            ...
            for step, (x_batch, y_batch, batch_size) in enumerate(train_batch_generator):
                tic = timer()
                train_batch_loss = self._train_batch(x_batch, y_batch, batch_size, epoch, step)
                toc = timer()
