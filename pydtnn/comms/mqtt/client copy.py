...
class Client(Protocol):
    ...
    def __init__(self, addr: str, port: int) -> None:
        ...
        self._register_handler(topic=f"s2c/{self._id}", handler=self._s2c)
        self._ini()

    def _ini(self) -> None:
        self._publish(topic=f"syc/{self._id}")
    ...
    def put(self, obj, *peers: uuid.UUID) -> None:
        ...
        self._publish(topic=f"c2s/{self._id}", data=data)
    ...
    def close(self) -> None:
        ...
        if self.closed:
            return
        super().close()
        self._publish(topic=f"fin/{self._id}")
