...
class ConcatenationBlock[T: Array](AbstractBlockLayer[T]):  # noqa: D101 (generics not detected)
    ...
    def _model_init(self, prev_shape: ArrayShape, x: T) -> None:
        ...
        match self.model.tensor_format:
            case TensorFormat.NCHW:
                ...
                if not all(
                    [
                        tuple(o[CONCAT_DIM_NCHW:]) == tuple(self.out_shapes[0][CONCAT_DIM_NCHW:])
                        for o in self.out_shapes
                    ]
                ):
                    raise LayerError(
                        f"All output shape must have the same number of elements.\n{
                            self.out_shapes
                        }"
                    )
            ...
            case TensorFormat.NHWC:
                ...
                if not all(
                    [
                        tuple(o[:CONCAT_DIM_NHWC]) == tuple(self.out_shapes[0][:CONCAT_DIM_NHWC])
                        for o in self.out_shapes
                    ]
                ):
                    raise LayerError(
            ...
            case tensor_format:
                raise NotImplementedError(
                    f"ConcatenationBlock is not implemented for {tensor_format} format."
                )
