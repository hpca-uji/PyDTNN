    ...
    def initialize(self, prev_shape, need_dx=True):
        ...
        if need_dx:
            self.bwd_time += matmul_time(m=(self.ci * self.kh * self.kw), n=(self.model.batch_size * self.ho * self.wo),
                                         k=self.co, cpu_speed=self.model.params.cpu_speed,
                                         memory_bw=self.model.params.memory_bw, dtype=self.model.dtype)
        else:
    ...
    def initialize(self, prev_shape, need_dx=True):
        ...
        for p in self.paths:
            for i, l in enumerate(p):
                l.tracer = self.model.tracer
                l.dtype = self.model.dtype
                l.model = self.model
                l.batch_size = self.model.batch_size
                l.id = self.model.id + i + 1
                l.matmul = self.model.matmul
