...
def _conv_direct_and_im2row_mm(weights: np.ndarray, x: np.ndarray, biases: np.ndarray | None = None,
    ...
    # b, c, h, w = x.shape
    conv_direct = ConvDirect(method_name="convdirect_original_nhwc_default", debug=verbose_test())
    cg_biases = biases.copy() if biases is not None else None
    conv_direct_result: np.ndarray = conv_direct.conv_direct(weights, x, biases=cg_biases,
...
class ConvDirectTestCase(TestCase):
    """
    ...
    def test_raise_on_different_strides(self):
        ...
        conv_direct_result, im2row_mm_result = _conv_direct_and_im2row_mm(weights, x,
                                                                          vpadding=d.vpadding, hpadding=d.hpadding,
                                                                          vstride=1, hstride=2,
                                                                          vdilation=d.vdilation, hdilation=d.hdilation)
        self.assertTrue(np.allclose(conv_direct_result, im2row_mm_result))
    ...
    def test_defaults_with_ones(self):
        ...
        weights = np.ones((d.c, d.kh, d.kw, d.kn)).astype(np.float32, order='C')
        x = np.ones((d.b, d.h, d.w, d.c)).astype(np.float32, order='C')
        conv_gemm_result, im2row_mm_result = _conv_direct_and_im2row_mm(weights, x,
                                                                        vpadding=d.vpadding, hpadding=d.hpadding,
    ...
    def test_defaults_including_biases_with_random(self):
        ...
        conv_gemm_result, im2row_mm_result = _conv_direct_and_im2row_mm(weights, x, biases=biases,
                                                                        vpadding=d.vpadding, hpadding=d.hpadding,
                                                                        vstride=d.vstride, hstride=d.hstride,
                                                                        vdilation=d.vdilation, hdilation=d.hdilation)
    ...
    def test_with_different_kn(self):
        ...
        if verbose_test():
            ...
            print("----+--------------------+-----------------")
        conv_gemm = ConvDirect(method_name="convdirect_original_nhwc_default", debug=False)
        x = random.random((d.b, d.h, d.w, d.c)).astype(np.float32, order='C')
        np_all_close_for_all_cases = True
        for kn in range(1, 32):
            weights = random.random((d.c, d.kh, d.kw, kn)).astype(np.float32, order='C')
            conv_gemm_result: np.ndarray = conv_gemm.conv_direct(weights, x,
                                                                 vpadding=d.vpadding, hpadding=d.hpadding,
                                                                 vstride=d.vstride, hstride=d.hstride,
                                                                 vdilation=d.vdilation, hdilation=d.hdilation)
    ...
    #         # deconv_gemm
    #         deconv_gemm_result: np.ndarray = conv_gemm.deconv_direct(weights, dy, dx,
    #                                                    vpadding=layer.vpadding, hpadding=layer.hpadding,
    #                                                    vstride=layer.vstride, hstride=layer.hstride,
    #                                                    vdilation=layer.vdilation, hdilation=layer.hdilation)
    #         # gemm + col2im
