...
class CheckConvGemmNCHWModels(_CheckConvGemmModels):
    """
    ...
    def get_model1_and_loss_func(model_name: str) -> tuple[Model, Loss]:
        model1, loss_func = _CheckConvGemmModels.get_model1_and_loss_func(model_name,
                                                                         overwrite_params={'tensor_format': TensorFormat.NCHW.upper()})
        return model1, loss_func

    @staticmethod
    def get_model2(model_name: str) -> Model:
        model2 = _CheckConvGemmModels.get_model2(model_name,
                                                overwrite_params={'tensor_format': TensorFormat.NCHW.upper()})
        return model2
