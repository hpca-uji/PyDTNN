...
def _conv_winograd_and_im2row_mm(weights: np.ndarray, x: np.ndarray, biases: np.ndarray | None = None,
    ...
    # b, c, h, w = x.shape
    conv_winograd = ConvWinograd(kh, kw, vstride, hstride, vdilation, hdilation, debug=verbose_test(), tensor_format=TensorFormat.NHWC)
    cw_biases = biases.copy() if biases is not None else None
    conv_winograd_result: np.ndarray = conv_winograd.conv_winograd_nhwc(weights, x, biases=cw_biases,
...
class ConvWinogradTestCase(TestCase):
    ...
    #     stride = 1
    #     dilation = 1
    #     conv_winograd_result, im2row_mm_result = _conv_gemm_and_im2row_mm(weights, x,
    #                                                                   vpadding=padding, hpadding=padding,
    #                                                                   vstride=stride, hstride=stride,
    #                                                                   vdilation=dilation, hdilation=dilation)
    #     if verbose_test():
    #         print(["{:b}  ".format(int(x)) for x in conv_winograd_result.ravel()])
    ...
    #     stride = 1
    #     dilation = 1
    #     conv_winograd_result, im2row_mm_result = _conv_gemm_and_im2row_mm(weights, x, biases=biases,
    #                                                                   vpadding=padding, hpadding=padding,
    #                                                                   vstride=stride, hstride=stride,
    #                                                                   vdilation=dilation, hdilation=dilation)
    #     if verbose_test():
    ...
    def test_defaults_including_biases_with_random(self):
        ...
        conv_winograd_result, im2row_mm_result = _conv_winograd_and_im2row_mm(weights, x, biases=biases,
                                                                              vpadding=d.vpadding, hpadding=d.hpadding,
                                                                              vstride=d.vstride, hstride=d.hstride,
                                                                              vdilation=d.vdilation, hdilation=d.hdilation)
        diff = conv_winograd_result - im2row_mm_result
        self.assertTrue(np.allclose(conv_winograd_result, im2row_mm_result), f"The difference is to big (rtol=1.e-5, atol=1.e-8). max diff: {diff.max()}. min diff: {diff.min()}")
    ...
    def test_with_different_kn(self):
        ...
        if verbose_test():
            ...
            print("----+--------------------+-----------------")
        conv_winograd = ConvWinograd(d.kh, d.kw, d.vstride, d.hstride, d.vdilation, d.hdilation, debug=False, tensor_format=TensorFormat.NHWC)
        x = random.random((d.b, d.h, d.w, d.c)).astype(np.float32, order='C')
        np_all_close_for_all_cases = True
        for kn in range(1, 32):
            weights = random.random((d.c, d.kh, d.kw, kn)).astype(np.float32, order='C')
            conv_winograd_result: np.ndarray = conv_winograd.conv_winograd_nhwc(weights, x,
                                                                                vpadding=d.vpadding, hpadding=d.hpadding,
                                                                                vstride=d.vstride, hstride=d.hstride,
                                                                                vdilation=d.vdilation, hdilation=d.hdilation)
            conv_winograd_result: np.ndarray = conv_winograd_result.reshape((-1, kn), copy=False)
            ...
            if verbose_test():
                print("{:3}    {:9.7f}             {:11.2f}"
                      "".format(kn, max([abs(x - y) for x, y in zip(conv_winograd_result.flatten(),
                                                                    im2row_mm_result.flatten())]),
                                np.sum(conv_winograd_result)))
            np_all_close_for_all_cases = np_all_close_for_all_cases and np.allclose(conv_winograd_result,
                                                                                    im2row_mm_result)
    ...
    def test_with_different_b(self):
        ...
        for b in range(1, 32):
            ...
            conv_winograd_result: np.ndarray = conv_winograd.conv_winograd_nhwc(weights, x,
                                                                                ...
                                                                                vstride=d.vstride, hstride=d.hstride,
                                                                                vdilation=d.vdilation, hdilation=d.hdilation)
            conv_winograd_result: np.ndarray = conv_winograd_result.reshape((-1, d.kn), copy=False)
            n, h, w, c = x.shape
            ...
            if verbose_test():
                print("{:3}    {:9.7f}".format(b,
                                               max([abs(x - y) for x, y
                                                    in
                                                    zip(conv_winograd_result.flatten(), im2row_mm_result.flatten())])))
            np_all_close_for_all_cases = np_all_close_for_all_cases and np.allclose(conv_winograd_result,
                                                                                    im2row_mm_result)
        self.assertTrue(np_all_close_for_all_cases)
            ...
            for hstride in range(1, 5):
                ...
                if verbose_test():
                    print("{:3} {:3}    {:9.7f}".format(vstride, hstride,
                                                        max([abs(x - y) for x, y
                                                             in
                                                             zip(conv_winograd_result.flatten(),
                                                                 im2row_mm_result.flatten())])))
                self.assertTrue(np.allclose(conv_winograd_result, im2row_mm_result),
                                f"Results differ with vstride {vstride} and hstride {hstride}")

    @unittest.skip("Winograd only support dilation (1, 1)")
    ...
    @unittest.skip("Winograd only support a subset of kernel sizes")
    def test_alexnet_layers(self):
        ...
        for n, layer in enumerate(layers):
            conv_winograd = ConvWinograd(layer.kh, layer.kw, layer.vstride, layer.hstride, layer.vdilation, layer.hdilation, debug=False, tensor_format=TensorFormat.NHWC)
            ...
            conv_winograd_result: np.ndarray = conv_winograd_result.reshape(-1, layer.kn)
            n, h, w, c = x.shape
                ...
                print("   {:2}      {:9.7f}".format(n,
                                                    max([abs(x - y) for x, y
                                                         ...
                                                         zip(conv_winograd_result.flatten(),
                                                             im2row_mm_result.flatten())])))
                if n == 9:
                    print("Flags for last conv_winograd_result output:")
                    print(conv_winograd_result.flags)
            self.assertTrue(np.allclose(conv_winograd_result, im2row_mm_result),
    ...
    #         print("-------+--------------------")
    #     layers = alexnet_layers
    #     conv_winograd = ConvWinograd(d.kh, d.kw, d.vstride, d.hstride, d.vdilation, d.hdilation, debug=False)
    #     for n, layer in enumerate(layers):
    #         weights = random.random((layer.c, layer.kh, layer.kw, layer.kn)).astype(np.float32, order='C')
    ...
    #         # deconv_gemm
    #         deconv_gemm_result: np.ndarray = conv_winograd.deconv_gemm_nhwc(weights, dy, dx,
    #                                                    vpadding=layer.vpadding, hpadding=layer.hpadding,
    #                                                    vstride=layer.vstride, hstride=layer.hstride,
    #                                                    vdilation=layer.vdilation, hdilation=layer.hdilation)
    #         # gemm + col2im
