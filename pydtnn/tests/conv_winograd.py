...
class ConvWinogradTestCase(ConvCommonTestCase):
    ...
    def _compute_both(
        ...
        conv_winograd_result: np.ndarray = cls._compute(
            weights, x.copy(), biases=cw_biases, kh=kh, kw=kw, vpadding=vpadding, hpadding=hpadding, vstride=vstride, hstride=hstride, vdilation=vdilation, hdilation=hdilation
        ).copy()
        ...
        if verbose_test():
            ...
            logger.info("Shape: ", conv_winograd_result.shape, " Sum: ", conv_winograd_result.sum(), " Min: ", conv_winograd_result.min(), " Max: ", conv_winograd_result.max())
            ...
            logger.info("---")
            logger.info("Maximum difference: ", max([abs(x - y) for x, y in zip(conv_winograd_result.flatten(), im2row_mm_result.flatten())]))
            logger.info("---")
    ...
    def _compute(weights: np.ndarray, x: np.ndarray, biases: np.ndarray | None = None, kh=1, kw=1, vpadding=0, hpadding=0, vstride=1, hstride=1, vdilation=1, hdilation=1):
        ...
        """
        return ConvWinograd(kh, kw, vstride, hstride, vdilation, hdilation, debug=verbose_test(), tensor_format=TensorFormat.NHWC).conv_winograd_nhwc(
