...
class ConvCommonTestCase(TestCase):
    ...
    def test_raise_on_different_strides(self) -> None:
        ...
        test_result, ref_result = self._compute_both(
            ...
            vstride=1,
            hstride=2,
            vdilation=d.vdilation,
            hdilation=d.hdilation,
        )
    ...
    def test_defaults_including_biases_with_random(self) -> None:
        ...
        self.assertTrue(
            np.allclose(test_result, ref_result),
            f"The difference is to big (rtol=1.e-5, atol=1.e-8). max diff: {diff.max()}. min diff:"
            f" {diff.min()}",
        )
                ...
                print(
                    "{:3}    {:9.7f}             {:11.2f}".format(
                        ...
                        max(
                            [
                                abs(x - y)
                                for x, y in zip(test_result.flatten(), ref_result.flatten())
                            ]
                        ),
                        np.sum(test_result),
                    )
                )
            np_all_close_for_all_cases = np_all_close_for_all_cases and np.allclose(
