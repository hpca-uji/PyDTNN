...
class ArgumentParser(argparse.ArgumentParser):
    ...
    def __init__(self):
        ...
        # Model
        self.add_argument("--model", dest="model_name", type=str, default=None, help="Neural network model: 'simplemlp', 'simplecnn', 'alexnet', 'vgg11', 'vgg16', etc. Default: 'None'.")
        self.add_argument(
            "--backend",
            type=str,
            default="cpu",
        ...
        self.add_argument(
            "--batch-size", type=int, default=None, help="Batch size per MPI rank. Or 'batch_size' or 'global_batch_size' must have a value different from 'None' (but not both). Default: 'None'."
        )
        self.add_argument(
            "--global-batch-size",
            type=int,
        ...
        self.add_argument("--dtype", type=np_dtype, default=np.float32, help="Datatype to use: 'float32', 'float64'. Default: 'float32'.")
        self.add_argument("--quantize", type=bool_lambda, default=False, help="Enable model quantization. Default: False.")
        self.add_argument("--quantize-dtype", type=np_dtype, default=np.float16, help="Quantized datatype to use: 'float32', 'float64'. Default: 'float16'.")
        self.add_argument("--num-epochs", type=int, default=1, help="Number of epochs to perform. Default: 1.")
        ...
        self.add_argument("--shared-tmp-memory", type=bool_lambda, default=False, help="Allows to use a memory pool for all the temporary data structures.")

        # Synchronization options
        _sy_group = self.add_argument_group("Synchronization options")
        _sy_group.add_argument("--shared-data", default=True, type=bool_lambda, help="If 'True' ranks assume they share the file system. Default: True.")
        ...
        _sy_group.add_argument(
            "--model-sync-algo", type=str, default="avg", choices=["avg", "wavg", "invwavg"], help="Aggregation method used to synchronize models: 'avg', 'wavg' or 'invwavg'. Default: 'avg'."
        )
        _sy_group.add_argument(
            "--model-sync-participation", type=str, default="all", choices=["all", "avail2all"], help="Rank participation to synchronize models: 'all' or 'avail2all'. Default: 'all'."
        )
        _sy_group.add_argument("--model-sync-min-avail", type=int, default=0, help="Minimum ranks with data required to synchronize models. Default: 0.")
        _sy_group.add_argument("--initial-model-sync", type=bool_lambda, default=True, help="Synchronize models on training start. Default: True.")
        _sy_group.add_argument("--final-model-sync", type=bool_lambda, default=True, help="Synchronize models on training end. Default: True.")
        _sy_group.add_argument("--model-sync-quantize", type=bool_lambda, default=False, help="Enable model quantization on synchronize. Default: False.")
        _sy_group.add_argument("--model-sync-dtype", type=np_dtype, default=np.float16, help="Model synchronization quantization target dtype. Default: float16.")

        # Dataset options
        _ds_group = self.add_argument_group("Dataset options")
        _ds_group.add_argument("--dataset", dest="dataset_name", type=str, default=None, help="Dataset to train: 'mnist', 'cifar10', 'synthetic', …. Default: 'None'.")
        ...
        _ds_group.add_argument("--dataset-path", type=str, default=_default_dataset_path, help="Path to the dataset.")
        _ds_group.add_argument("--dataset-lang", type=str, default="en", help="Dataset language.")
        _ds_group.add_argument("--dataset-lang2", type=str, default="de", help="Dataset second language.")
        _ds_group.add_argument("--synthetic-train-samples", default=1000, type=int, help="Number of synthetic train sample. Default: 1000.")
        _ds_group.add_argument("--synthetic-test-samples", default=100, type=int, help="Number of synthetic train sample. Default: 100.")
        _ds_group.add_argument("--synthetic-input-shape", default="3,32,32", type=str, help="Synthetic input shape (coma separated). Default: 3,32,32.")
        _ds_group.add_argument("--synthetic-output-shape", default="10", type=str, help="Synthetic output shape (coma separated). Default: 10.")
        _ds_group.add_argument(
            "--test-as-validation", default=False, type=bool_lambda, help="Prevent making partitions on training data for training+validation data, use test data for validation. True if specified."
        )
        _ds_group.add_argument("--validation-split", type=factor, default=0.2, help="Split between training and validation data.")
        _ds_group.add_argument("--augment-shuffle", default=True, type=bool_lambda, help="Shuffle training images. Default: True.")
        _ds_group.add_argument("--augment-horizontal-flip", type=factor, default=0.0, help="Probability to do a horizontal flip to the training images. If the value is less or equal to 0 it is disabled. Default: 0.0.")
        _ds_group.add_argument("--augment-vertical-flip", type=factor, default=0.0, help="Probability to do a vertical flip to the training images. If the value is less or equal to 0 it is disabled. Default: 0.0.")
        ...
        _ds_group.add_argument("--augment-rotate-degree", type=float, default=90.0, help="The maximum degree to rotate training images.Default: 90.0.")
        _ds_group.add_argument("--augment-brightness", type=factor, default=0.0, help="Probability to change the brightness to training images. If the value is less or equal to 0 it is disabled. Default: 0.0.")
        _ds_group.add_argument("--augment-brightness-factor", type=float, default=1.0, help="The maximum brightness to apply in training images. Value ranges from 0 (no brightness), to 1 (same), up to infinity. Default: 0.0.")
        _ds_group.add_argument("--augment-contrast", type=factor, default=0.0, help="Probability to change the contrast to training images. If the value is less or equal to 0 it is disabled. Default: 0.0.")
        _ds_group.add_argument("--augment-contrast-factor", type=float, default=1.0, help="The maximum contrast to apply in training images. Value ranges from 0 (no brightness), to 1 (same), up to infinity. Default: 0.")
        _ds_group.add_argument("--augment-saturation", type=factor, default=0.0, help="Probability to change the saturation to training images. If the value is less or equal to 0 it is disabled. Default: 0.0.")
        _ds_group.add_argument("--augment-saturation-factor", type=float, default=1.0, help="The maximum saturation to apply in training images. Value ranges from 0 (no brightness), to 1 (same), up to infinity. Default: 0.")
        ...
        _ds_group.add_argument("--augment-crop", default=False, type=bool_lambda, help="Crop the images. True if specified.")
        ...
        _ds_group.add_argument("--augment-perspective", type=factor, default=0.0, help="Probability to change the perspective in training images. If the value is less or equal to 0 it is disabled. Default: 0.0.")
        ...
        _oo_group.add_argument("--enable-fused-conv-bn-relu", type=bool_lambda, default=False, help="Fuse Conv2D and BatchNormalization and Relu layers. Default: False.")
        ...
        _op_group.add_argument("--optimizer", dest="optimizer_name", type=str, default="sgd", help="Optimizers: 'sgd', 'rmsprop', 'adam', 'nadam', 'oktopk'. Default: 'sgd'. ")
        _op_group.add_argument("--learning-rate", type=float, default=1e-2, help="Learning rate. Default: 0.01.")
        _op_group.add_argument("--learning-rate-scaling", default=False, type=bool_lambda, help="Scale learning rate in data parallelism: new_lr = lr / num_procs.  True if specified.")
        _op_group.add_argument("--optimizer-momentum", type=float, default=0.9, help="Decay rate for optimizers. Default: 0.9.")
        _op_group.add_argument("--optimizer-decay", type=float, default=0.0, help="Decay rate for optimizers. Default: 0.0.")
        _op_group.add_argument("--optimizer-nesterov", default=False, type=bool_lambda, help="Whether to apply Nesterov momentum. Default: False.")
        _op_group.add_argument("--optimizer-beta1", type=float, default=0.99, help="Variable for 'adam', 'nadam' optimizers. Default: 0.99.")
        _op_group.add_argument("--optimizer-beta2", type=float, default=0.999, help="Variable for 'adam', 'nadam' optimizers. Default: 0.999.")
        _op_group.add_argument("--optimizer-epsilon", type=float, default=1e-7, help="Variable for 'rmsprop', 'adam', 'nadam'. Default=1e-7.")
        _op_group.add_argument("--optimizer-rho", type=float, default=0.9, help="Variable for 'rmsprop' optimizers. Default: 0.99.")
        _op_group.add_argument("--optimizer-tau", type=int, default=64, help="Variable for 'oktopk' optimizers. Default: 64.")
        _op_group.add_argument("--optimizer-tau-prime", type=int, default=32, help="Variable for 'oktopk' optimizers. Default: 32.")
        _op_group.add_argument("--optimizer-density", type=float, default=0.01, help="Variable for 'oktopk' optimizers. Default: 0.01.")
        ...
        _sh_group.add_argument("--warm-up-epochs", type=int, default=5, help="Number of batches (ramp up) that the LR is scaled up from 0 until LR. Default: 5.")
        _sh_group.add_argument(
            "--early-stopping-metric", type=str, default="val_categorical_cross_entropy", help="Loss metric monitored by early_stopping LR scheduler. Default: 'val_categorical_cross_entropy'."
        )
        _sh_group.add_argument("--early-stopping-patience", type=int, default=10, help="Number of epochs with no improvement after which training will be stopped. Default: 10.")
        _sh_group.add_argument("--early-stopping-minimize", type=bool_lambda, default=True, help="Whether to minimize the metric. If False, it will maximize. Default: True.")
        _sh_group.add_argument(
        ...
        _sh_group.add_argument("--reduce-lr-on-plateau-factor", type=float, default=0.1, help="Factor by which the learning rate will be reduced. new_lr = lr * factor. Default: 0.1.")
        _sh_group.add_argument("--reduce-lr-on-plateau-patience", type=int, default=5, help="Number of epochs with no improvement after which LR will be reduced. Default: 5.")
        _sh_group.add_argument("--reduce-lr-on-plateau-min-lr", type=float, default=0, help="Lower bound on the learning rate. Default: 0.")
        _sh_group.add_argument("--reduce-lr-every-nepochs-factor", type=float, default=0.1, help="Factor by which the learning rate will be reduced. new_lr = lr * factor. Default: 0.1.")
        _sh_group.add_argument("--reduce-lr-every-nepochs-nepochs", type=int, default=5, help="Number of epochs after which LR will be periodically reduced. Default: 5.")
        _sh_group.add_argument("--reduce-lr-every-nepochs-min-lr", type=float, default=0, help="Lower bound on the learning rate. Default: 0.")
        _sh_group.add_argument("--stop-at-loss-metric", type=str, default="val_accuracy", help="Loss metric monitored by stop_at_loss LR scheduler. Default: 'val_accuracy'.")
        _sh_group.add_argument("--stop-at-loss-threshold", type=float, default=0, help="Metric threshold monitored by stop_at_loss LR scheduler. Default: 0.")
        _sh_group.add_argument(
            "--model-checkpoint-metric", type=str, default="val_categorical_cross_entropy", help="Loss metric monitored by model_checkpoint LR scheduler. Default: 'val_categorical_cross_entropy'"
        )
        _sh_group.add_argument(
            "--model-checkpoint-save-freq", type=int, default=2, help="Frequency (in epochs) at which the model weights and bias will be saved by the model_checkpoint LR scheduler. Default: 2."
        )

        # Parallel execution options
        _pe_group = self.add_argument_group("Parallel execution options")
        _pe_group.add_argument("--parallel-data", type=bool_lambda, default=False, help="Enable data parallelization modes. Default: 'False'.")
        _pe_group.add_argument("--parallel-pipeline", type=bool_lambda, default=False, help="Enable pipeline parallelization modes. Default: 'False'.")
        _pe_group.add_argument("--use-blocking-mpi", type=bool_lambda, default=True, help="Enable non-blocking MPI primitives. Default: True.")
        ...
        _pe_group.add_argument("--enable-cudnn", type=bool_lambda, default=None, help="Ignored, always enabled if plausible, present just for compatibility.")
        _pe_group.add_argument("--enable-gpudirect", type=bool_lambda, default=False, help="Enable GPU pinned memory for gradients when using a CUDA-aware MPI version. Default: False.")
        _pe_group.add_argument(
            "--enable-nccl",
            type=bool_lambda,
            default=False,
        ...
        _cy_group.add_argument("--encryption", dest="encryption_name", type=str, default="", help="Encryption library: 'tenseal', 'openfhe', '' (None). Default '' (None).")
        _cy_group.add_argument("--encryption-slots", type=int, default=13, help="Encryption slot count. 2 ^ 'value'. Default: 13.")
        _cy_group.add_argument("--encryption-scale", type=int, default=40, help="Encryption operational scale. 2 ^ 'value''. Default: 40.")
        _cy_group.add_argument("--encryption-security", type=int, default=128, help="Encryption security level: 128, 192, 256. Default: 128.")

        # Tracing and profiling
        _tr_group = self.add_argument_group("Tracing options")
        _tr_group.add_argument("--tracing", type=bool_lambda, default=False, help="Obtain Simple/Extrae-based traces. Deffault: False.")
        _tr_group.add_argument("--tracer-output", type=str, default="", help="Output file to store the Simple/Extrae-based traces.")
        _tr_group.add_argument("--tracer-pmlib-server", type=str, default="127.0.0.1", help="Address of PMlib tracer server. Default: '127.0.0.1'.")
