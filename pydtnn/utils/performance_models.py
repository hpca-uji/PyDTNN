...
def flops2time(
    flops: float, memops: int, cpu_speed: float, memory_bw: float, dtype: type | np.dtype
...
def im2col_time(
    m: int, n: int, cpu_speed: float, memory_bw: float, dtype: type | np.dtype
) -> np.ndarray[tuple[int, int, int, int], np.dtype[np.float32]]:
...
def matmul_time(
    m: int, n: int, k: int, cpu_speed: float, memory_bw: float, dtype: type | np.dtype
) -> np.ndarray[tuple[int, int, int, int], np.dtype[np.float32]]:
...
def bcast_time(
    ...
    match network_algo:
        ...
        case NetworkAlgoEnum.VDG:
            time = (log(nprocs, 2) + nprocs - 1.0) * (network_lat) + 2.0 * (
                (nprocs - 1.0) / nprocs
...
def scatter_time(
    ...
    match network_algo:
        case NetworkAlgoEnum.BTA:
            time = ceil(log(nprocs, 2)) * network_lat + ((nprocs - 1) / nprocs) * (
                (elems * bfp * 8.0) / network_bw
            )
        case NetworkAlgoEnum.VDG:
            time = log(nprocs) * (network_lat) + ((nprocs - 1) / nprocs) * (
                (elems * bfp * 8.0) / network_bw
            )
        case _:
            raise ValueError(f"network_algo ({network_algo}) not in {list(NetworkAlgoEnum)}")
...
def allgather_time(
    ...
    match network_algo:
        case NetworkAlgoEnum.BTA:
            time = (nprocs - 1) * network_lat + ((nprocs - 1) / nprocs) * (
                (elems * bfp * 8.0) / network_bw
            )
        case NetworkAlgoEnum.VDG:
