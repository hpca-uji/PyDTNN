"""
ResNet-110 model architecture implementation for PyDTNN.
"""
...
def resnet110(input_shape: ArrayShape, output_shape: ArrayShape) -> Sequence[Layerable]:
    """
    Constructs a ResNet-110 model architecture.
    ...
    Returns:
        A sequence of layers forming the ResNet-110 model.
    """
        ...
        for r in range(res_blocks):
            ...
            _(
                AdditionBlock(
                    ...
                    [Conv2D(nfilters=n_filt, filter_shape=(1, 1), stride=stride, weights_initializer=he_uniform), BatchNormalization()] if r == 0 or stride != 1 else [],
                )
            )
            _(Relu())
