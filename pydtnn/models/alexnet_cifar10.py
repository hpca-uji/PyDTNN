...
def create_alexnet_cifar10(input_shape: Sequence[int], output_shape: Sequence[int]) -> Iterable[LayerAndActivationBase]:
    yield Input(shape=input_shape)
    yield Conv2D(nfilters=64, filter_shape=(3, 3), padding=1, stride=2, activation=relu)
    yield MaxPool2D(pool_shape=(2, 2), stride=2)
    yield Conv2D(nfilters=192, filter_shape=(3, 3), padding=1, stride=1, activation=relu)
    yield MaxPool2D(pool_shape=(2, 2), stride=2)
