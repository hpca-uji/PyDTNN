...
from . import LayerGPUMixin
from ..tensor_gpu import TensorGPU
from ...performance_models import im2col_time, col2im_time


class Pool2DGPUMixin(LayerGPUMixin):
    """
    Mixin that provides common methods to Pool2DGPU classes.
    """

    def __init__(self, *args, **kwargs):
        # Attributes that will be defined on super().__init__(), on pool_2d_gpu_initialize, or elsewhere.
        self.id = None
        self.pool_desc = None
        self.pool_shape = None
        self.ci = self.hi = self.wi = self.kh = self.kw = self.co = self.ci = None
        self.ho = self.wo = None
        self.shape = self.vpadding = self.hpadding = self.vstride = self.hstride = None
        self.model = self.need_dx = None
        self.fwd_time = self.bwd_time = None
        # super().__init__() must be called AFTER the previous declarations
        super().__init__(*args, **kwargs)

    def pool_2d_gpu_initialize(self, prev_shape, need_dx, x, pool_mode):
