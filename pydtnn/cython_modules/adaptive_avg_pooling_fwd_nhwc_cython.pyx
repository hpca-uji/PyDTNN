...
@cython.boundscheck(False)
@cython.wraparound(False)
def adaptive_avg_pooling_fwd_nhwc_cython(np.ndarray x, int output_h, int output_w) -> np.ndarray:
    cdef int n = x.shape[0]
    cdef int h = x.shape[1]
    cdef int w = x.shape[2]
    cdef int c = x.shape[3]        

    cdef np.ndarray pooled_x = np.empty((n, output_h, output_w, c), dtype = x.dtype)
