...
cdef avg_pooling(np.ndarray[npDT, ndim=4] pooled_x,
...
def adaptive_avg_pooling_fwd_nhwc_cython(np.ndarray[npDT, ndim=4] x, 
    ...
    cdef np.ndarray[npDT, ndim=4] pooled_x = np.empty((n, new_h, new_w, c), dtype = x.dtype)
...
def adaptive_avg_pooling_bwd_nhwc_cython(np.ndarray[npDT, ndim=4] dy, 
                                         int new_h, int new_w) -> np.ndarray:
    ...
    cdef np.ndarray[npDT, ndim=4] dx = np.empty((n, new_h, new_w, c), dtype = dy.dtype)

    try:
        backward_avg_pooling(dy, dx, n, c, h, w, new_h, new_w)
        return dx
    except TypeError as e:
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef backward_avg_pooling(np.ndarray[npDT, ndim=4] dx,
