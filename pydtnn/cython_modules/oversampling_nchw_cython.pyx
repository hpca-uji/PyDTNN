...
def oversampling_fwd_nchw_cython(np.ndarray x, int new_h, int new_w, 
    ...
    cdef dict pseudo_switch = PSEUDO_SWITCH_FUNCTIONS

    cdef np.ndarray oversampled_x = np.empty((n, c, new_h, new_w), dtype = x.dtype)

    if x.dtype in PSEUDO_SWITCH_FUNCTIONS:
        return pseudo_switch[x.dtype](oversampled_x, x, n, c, h, w, extra_h, extra_w)
    else: 
        raise TypeError(f" Type received '{x.dtype}'. Types expected in oversampling_fwd_nchw_cython: {PSEUDO_SWITCH_FUNCTIONS.keys()}")
