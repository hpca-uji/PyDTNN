...
def argmax_cython(np.ndarray[supported_types_t, ndim=2] x, 
                  int axis=0) -> tuple(np.ndarray, tuple(np.ndarray, np.ndarray)):
    
    if axis == 0: x = x.T

    cdef np.ndarray[supported_types_t, ndim=1] maxv = np.empty((x.shape[0],), dtype=x.dtype)
    cdef np.ndarray[np.int32_t, ndim=1] amax = np.empty((x.shape[0],), dtype=np.int32)
    cdef np.ndarray[np.int32_t, ndim=1] rng = np.empty((x.shape[0],), dtype=np.int32)
    ...
    try:
        argmax_cython_inner(x_view, maxv_view, amax_view, rng_view, minval)
...
cdef argmax_cython_inner(const supported_types_t[:, :] x,
