...
def argmax_cython(np.ndarray[npDT, ndim=2] x, int axis=0) -> tuple(np.ndarray, tuple(np.ndarray, np.ndarray)):
    
    if axis == 0: x = x.T

    cdef np.ndarray[npDT, ndim=1] maxv = np.empty((x.shape[0],), dtype=x.dtype)
    ...
    try:
        argmax_cython_inner(x, maxv, amax, rng, minval)
...
cdef argmax_cython_inner(np.ndarray[npDT, ndim=2] x,
