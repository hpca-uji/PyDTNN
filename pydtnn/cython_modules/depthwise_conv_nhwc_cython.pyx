...
@cython.boundscheck(False)
@cython.wraparound(False)
def depthwise_conv_nhwc_cython(np.ndarray x, np.ndarray k, int vpadding, int hpadding, 
    ...
    cdef np.ndarray res = np.zeros((n, hi, ww, c), dtype=x.dtype)
    try:
...
@cython.boundscheck(False)
@cython.wraparound(False)
def depthwise_conv_backward_nhwc_cython(np.ndarray dy, np.ndarray k, np.ndarray x, 
                                        int vpadding, int hpadding, 
                                        int vstride, int hstride, 
    ...
    cdef np.ndarray dx = np.zeros((n, h, w, c), dtype=x.dtype)
