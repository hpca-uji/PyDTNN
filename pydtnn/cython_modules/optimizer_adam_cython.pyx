...
def adam_cython(w: np.ndarray, dw: np.ndarray, m: np.ndarray, v: np.ndarray,
    ...
    try:
        adam(w.reshape(-1, copy=False), dw.reshape(-1, copy=False), 
             m.reshape(-1, copy=False), v.reshape(-1, copy=False), 
             lr, beta1, beta2, epsilon, decay, it, num_elemts)
    except TypeError as e:
        raise TypeError(f"Function: \"adam_cython\". Error: {e}")
# --- END adam_cython --- #

def adam(np.ndarray[npDT, ndim=1] w, np.ndarray[npDT, ndim=1] dw, 
    ...
    _adam(w_view, dw_view, m_view, v_view, lr, beta1, beta2, epsilon, decay, it, num_elemts)
# --- END adam --- #
...
@cython.cdivision
cdef _adam(npDT[:] w, npDT[:] dw, npDT[:] m, npDT[:] v, 
           float lr, float beta1, float beta2, float epsilon, 
           float decay, int it, int num_elemts):
