...
def adaptive_avg_pooling_fwd_nchw_cython(np.ndarray[supported_types_t, ndim=4] x, 
    ...
    cdef np.ndarray[supported_types_t, ndim=4] pooled_x = np.empty((n, c, new_h, new_w), dtype = x.dtype)
...
def adaptive_avg_pooling_bwd_nchw_cython(np.ndarray[supported_types_t, ndim=4] dy, 
                                         int new_h, int new_w) -> np.ndarray:
    ...
    cdef np.ndarray[supported_types_t, ndim=4] dx = np.empty((n, c, new_h, new_w), dtype = dy.dtype)
