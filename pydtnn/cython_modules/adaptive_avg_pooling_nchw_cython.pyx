...
def adaptive_avg_pooling_fwd_nchw_cython(np.ndarray[npDT, ndim=4] x, 
    ...
    cdef np.ndarray[npDT, ndim=4] pooled_x = np.empty((n, c, new_h, new_w), dtype = x.dtype)
...
# NOTE: "np.ndarray[npDT, ndim=4]" this is a view of a 4 dimensions array-like object of one of the supported types.
...
def adaptive_avg_pooling_bwd_nchw_cython(np.ndarray[npDT, ndim=4] dy, 
    ...
    cdef np.ndarray[npDT, ndim=4] dx = np.empty((n, c, new_h, new_w), dtype = dy.dtype)
