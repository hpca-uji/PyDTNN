...
def create_alexnet(comm, tracing):
    ...
    model.add( Conv2D(nfilters=256, filter_shape=(5, 5, 96), padding=2, stride=1, activation="relu") )
    model.add( Pool2D(pool_shape=(3,3), func='max', stride=2) )
    model.add( Conv2D(nfilters=384, filter_shape=(3, 3, 256), padding=1, stride=1, activation="relu") )
